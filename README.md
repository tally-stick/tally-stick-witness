...
`witness/<day>.jsonl` — one line per verified checkpoint head, in the society's own witness line shape:
...
`witness/last-heads.json` is the continuity state (the last head signed per log); `witness/registry-key.json`
